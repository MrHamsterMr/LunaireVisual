...
public class ModuleManager {
    ...
    public static void init() {
        ...
        // №14 FastSwap - быстрая смена на выбранный слот (настройка в ">")
        LunaireModule fastSwap = new LunaireModule("FastSwap", Category.COMBAT, 0) {
            @Override
            public void onTick() {
                Setting s = getSetting("Slot");
                if (mc.player != null && s != null) {
                    // Переключает слот в хотбаре на тот, что в настройках
                    mc.player.getInventory().selectedSlot = (int)s.dVal - 1;
                }
            }
        };
        fastSwap.addSetting(new Setting("Slot", 1.0)); // Можно выбрать от 1 до 9
        ...
        // №11 HitColor - кастомный цвет удара (настройка в ">")
        ...
        // №20 NoRender - отключение лишнего (настройка в ">")
        ...
        modules.add(new LunaireModule("TargetHUD", Category.VISUAL, 0) {
            ...
            public void onRenderHud(DrawContext context) {
                if (mc.targetedEntity instanceof LivingEntity target) {
                    ...
                    // Отрисовка брони цели и её прочности
                    int ax = 5;
                    for (ItemStack s : target.getArmorItems()) {
                        if (!s.isEmpty()) { 
                            context.drawItem(s, x + ax, y + 25); 
                            int dur = s.getMaxDamage() - s.getDamage();
                            context.drawText(mc.textRenderer, String.valueOf(dur), x + ax, y + 42, 0xAAAAAA, false);
                            ax += 25; 
                        }
                    }
                }
        ...
        // №10 Friends - настройка друзей (настройка в ">")
        LunaireModule friends = new LunaireModule("Friends", Category.MISC, 0) {};
        friends.addSetting(new Setting("ShowArmor", true));
        modules.add(friends);
        ...
        // №5 ArmorHUD - прочность твоей брони цифрами
